...
const TrendingMoviesWeek = () => {
    ...
    useEffect(() => {
        axios.get("/trending/movie/week")
            .then(response => {
                ...
                context.setTrendingMoviesWeek(response.data.results)
            })
            .catch(err => {
                console.log(err)
            })
    ...
    const handleTrendingMoviesWeek = (movie) => {
        setMovieId(movie.id)
        setRedirect(true)
    }
    ...
    return (
        <div>
            ...
            <section className="hr_popular_cards">
                {
                    context.loading && context.trendingMoviesWeek.map((movie, i) => {
                        return (
                            <div className="sp_card" key={i} onClick={() => handleTrendingMoviesWeek(movie)}>
                                <div>
                                    <img
